...
// feed (boot.ts), and a single applyRoute() that hands the resulting entry to the four subsystems.
...
function forgetEntry(slug: string): void {
  ...
  setEntries(remaining, 'ready');
  applyRoute(router.current());
  applySession();
}

/**
 * The catalogue changed: the one place it is written, and handed to the three surfaces that hold a copy
 * of it — the strip, the Library grid and the menu (which also learns whether the feed is there at all).
 */
function setEntries(next: readonly CollectionEntry[], state: ListState): void {
  entries = next;
  carousel.setEntries(next);
  libraryScreen.setEntries(next);
  controls.setCollection(state, next);
}
